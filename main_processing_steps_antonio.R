...
rna_expr_data_C <- read.table("/Users/antoniozappia/Downloads/DE-HW1-main/TCGA-THCA - Counts/TCGA-THCA_rna_expr_data_C.txt", header=TRUE, sep="", check.names = F) 
#View(rna_expr_data_C)

rna_gene_data_C <- read.table("/Users/antoniozappia/Downloads/DE-HW1-main/TCGA-THCA - Counts/TCGA-THCA_rna_genes_C.txt", header=FALSE, sep="", check.names = F) 
# View(rna_gene_data_C)

rna_patients_data_C <- read.table("/Users/antoniozappia/Downloads/DE-HW1-main/TCGA-THCA - Counts/TCGA-THCA_rna_patients_C.txt", header=FALSE, sep="", check.names = F) 
# View(rna_patients_data_C)

# read the normal (natural) TCGA-THCA dataset [TSS - 11]
rna_expr_data_N <- read.table("/Users/antoniozappia/Downloads/DE-HW1-main/TCGA-THCA - Counts/TCGA-THCA_rna_expr_data_N.txt", header=TRUE, sep="", check.names = F) 
# View(rna_expr_data_C)

rna_gene_data_N <- read.table("/Users/antoniozappia/Downloads/DE-HW1-main/TCGA-THCA - Counts/TCGA-THCA_rna_genes_N.txt", header=FALSE, sep="", check.names = F) 
# View(rna_gene_data_C)

rna_patients_data_N <- read.table("/Users/antoniozappia/Downloads/DE-HW1-main/TCGA-THCA - Counts/TCGA-THCA_rna_patients_N.txt", header=FALSE, sep="", check.names = F) 
# View(rna_patients_data_C)

# to be use (?) - for extra information, i think.. now its useless
clinical_data <- read.csv2("/Users/antoniozappia/Downloads/DE-HW1-main/TCGA-THCA - Counts/TCGA-THCA_clinical_data.txt", header = FALSE, sep=",")
...
plot(gC, vertex.size=5, edge.curverd=.1, arrow.size=.1, vertex.color = "red", main = paste("Co-expression network in TUMOR", sep = " "),
...
plot(gN, vertex.size=5, edge.curverd=.1, arrow.size=.1, vertex.color = "green", main = paste("Co-expression network in NORMAL", sep = " "),
     arrow.width=.1, edge.arrow.size=.1, layout= layout.kamada.kawai, vertex.label = NA) # , vertex.label.dist = .8 and .y, vertex.label.cex=1

# degree distribution of the graphs
dgC <- degree(gC)
dgN <- degree(gN)
hist(dgC[dgC != 0], main = paste("Degree distribution in TUMOR condition", sep = " ") , col = "red", xlab = "Degree Distribution")
hist(dgC[dgC != 0], main = paste("Degree distribution in NORMAL condition", sep = " "), col = "green", xlab = "Degree Distribution")
...
# Ricreate the correlation datasets respect pearson correlation method
co_net_corr_dataC <- cor(t(rna_expr_data_C), method = "pearson")
co_net_corr_dataN <- cor(t(rna_expr_data_N), method = "pearson")
...
# Applying Z scores function
ZcoData <- (ZcoDataC-ZcoDataN)/sqrt((1/(nrow(rna_expr_data_C)-3)) + (1/(nrow(rna_expr_data_N)-3)))

# Threshold it
tshZ <- 13 # 13 is the best for getting a scale-free, 3 is too bad. Qe obtain that after some trial in which we analyze the degree distribution
...
#third Extra part in which we use two different correlation method
